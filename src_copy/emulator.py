...
class CPU:
    def __init__(self, code: bytes, data: bytes, input_tokens: list = None, trace=True, max_cycles=500_000):
    ...
    def _ea_write(self, mode, reg, val, sz=False):
        ...
        if mode == AM.MEM_DISP:
            disp = self._s32(self._fetch())
            self._mwrite(self.A[reg - 8] + disp, val)
            return
        if mode == AM.MEM_IDX:
            disp = self._s32(self._fetch())
            xreg = self._fetch()
            xval = self.D[xreg] if xreg < 8 else self.A[xreg - 8]
            self._mwrite(self.A[reg - 8] + disp + xval, val)
            return
    ...
    def step(self):
        ...
        if self.trace:
            ...
            r = (
                f"D0={self.D[0]:08X} D1={self.D[1]:08X} D2={self.D[2]:08X} "
