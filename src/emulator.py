...
class CPU:
    ...
    def _ea_write(self, mode, reg, val, sz=False):
        ...
        elif mode == AM.MEM_IND: self._mwrite(self.A[reg - 8], val)
    ...
    def _exec(self, op, sm, dm, sr, dr, sz) -> str:
        ...
        if op == Op.LINK:
            ...
            self._push(self.A[an]); self.A[an] = self.A[7]
        ...
        if op in BRANCHES:
            ...
            taken = {
                Op.JMP: True,          Op.JSR: True,
                Op.BEQ: f['Z'] == 1,   Op.BNE: f['Z'] == 0,
                Op.BLT: f['N'] != f['V'],
                Op.BGT: f['Z'] == 0 and f['N'] == f['V'],
                Op.BLE: f['Z'] == 1 or  f['N'] != f['V'],
                Op.BGE: f['N'] == f['V'],
                Op.BMI: f['N'] == 1,   Op.BPL: f['N'] == 0,
                Op.BCC: f['C'] == 0,   Op.BCS: f['C'] == 1,
                Op.BVC: f['V'] == 0,   Op.BVS: f['V'] == 1,
                Op.BRA: True,
            }[op]
        ...
        if op == Op.ADD:
            ...
            self._set_nz(new, sz); return "add"

        if op == Op.SUB:
            src = self._ea_read(sm, sr, sz)
            old, new = self._rmw(dm, dr, lambda x: x - src, sz)
            self.SR['C'] = 1 if self._u32(src) > self._u32(old) else 0
            self._set_nz(new, sz); return "sub"

        if op == Op.MUL:
            src = self._s32(self._ea_read(sm, sr, sz))
            res = self._s32(self.D[dr]) * src
            self.D[dr] = self._u32(res); self._set_nz(res, sz); return "mul"

        if op == Op.DIV:
            src = self._s32(self._ea_read(sm, sr, sz))
            if src == 0: raise RuntimeError("Деление на ноль")
            res = int(self._s32(self.D[dr]) / src)
            self.D[dr] = self._u32(res); self._set_nz(res, sz); return "div"

        if op == Op.CMP:
            src = self._s32(self._ea_read(sm, sr, sz))
            if dm == AM.REG_D:   dv = self._s32(self.D[dr])
            elif dm == AM.REG_A: dv = self._s32(self.A[dr - 8])
            else:                dv = self._s32(self._mread(self._ea_addr(dm, dr)))
            res = dv - src
        ...
        if op == Op.ASL:
            ...
            self._set_nz(self.D[dr], sz); return "asl"
        if op == Op.ASR:
            cnt = self._ea_read(sm, sr, sz)
            self.D[dr] = self._u32(self._s32(self.D[dr]) >> cnt)
            self._set_nz(self.D[dr], sz); return "asr"
        if op == Op.LSL:
            cnt = self._ea_read(sm, sr, sz)
            self.D[dr] = self._u32(self.D[dr] << cnt)
            self._set_nz(self.D[dr], sz); return "lsl"
        if op == Op.LSR:
            cnt = self._ea_read(sm, sr, sz)
            self.D[dr] = self._u32(self.D[dr] >> cnt)
            self._set_nz(self.D[dr], sz); return "lsr"
    ...
    def run(self, start_pc: int = 0):
        ...
        if self.cycle >= self.max_cycles and not self.halted:
            print(
                f"[EMU] Достигнут лимит тактов ({self.max_cycles}). "
                f"Используйте --max-cycles для увеличения.",
                file=sys.stderr,
            )
    ...
    def dump_state(self) -> str:
        ...
        for i in range(8):
            lines.append(
                f"  D{i}={self.D[i]:08X} ({self._s32(self.D[i]):12d})   "
                f"A{i}={self.A[i]:08X}"
            )
...
def locate_files(prog_name: str, search_dirs: list[str]) -> tuple[str, str, str]:
    ...
    raise FileNotFoundError(
        f"Не найдены файлы программы '{stem}' ни в одной из директорий: "
        + ', '.join(search_dirs)
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            f'Директория, где искать .imem / .dmem / .labels '
            f'(по умолчанию: {DEFAULT_OUT_DIR!r} из config.py).'
        ),
    )
    ...
    parser.add_argument(
        ...
        default=200,
        help=(
            'Максимальное число строк трассировки, выводимых на экран '
            '(по умолчанию: 200). Не влияет на --trace-out.'
        ),
    )
    parser.add_argument(
        '--trace-out',
...
def main():
    ...
    if stdin_source is None:
        ...
        print(f"[EMU] Ввод      : интерактивный stdin")
    if args.trace:
