...
class CPU:
    ...
    def _exec(self, op, sm, dm, sr, dr, sz):
        ...
        if op == Op.MOVE:
            v = self._read(sm, sr, sz)
            self._write(dm, dr, v, sz)
            self._nz(v, sz)
            return "move"
        if op == Op.MOVEA:
            self.A[dr - 8] = self._u32(self._read(sm, sr, sz))
            return "movea"
        ...
        if op == Op.CMP:
            src = self._s32(self._read(sm, sr, sz))
            dv = self._s32(
                self.D[dr] if dm == AM.REG_D else self.A[dr - 8] if dm == AM.REG_A else self._mr(self._addr(dm, dr))
            )
            res = dv - src
        ...
        if op == Op.NOT:
            ...
            self._nz(self.D[sr], sz)
            return "not"
        if op == Op.ASL:
            cnt = self._read(sm, sr, sz)
...
Примеры:
  ...
  python run_hello.py --input "Bob" --trace --trace-limit 50
    ...
    parser.add_argument(
        "--input", "-i", default=None, help="Входная строка (имя пользователя). Без флага — читать с stdin."
    )
    parser.add_argument("--trace", "-t", action="store_true", help="Печатать трассировку тактов.")
    parser.add_argument(
        "--trace-limit", type=int, default=200, metavar="N", help="Максимум строк трассировки (по умолчанию 200)."
    )
    parser.add_argument(
        "--max-cycles", type=int, default=200_000, metavar="N", help="Лимит тактов (по умолчанию 200000)."
    )
